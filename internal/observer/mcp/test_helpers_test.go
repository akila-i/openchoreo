...
func newTestMCPHandler(t *testing.T, opts ...func(*handlerTestDeps)) *MCPHandler {
	...
	d := handlerTestDeps{
		...
		platform: servicemocks.NewMockPlatformLogsQuerier(t),
	}
	for _, o := range opts {
		o(&d)
	}
	...
	h, err := NewMCPHandler(healthSvc, d.logs, d.events, d.metrics, d.alerts, d.traces, d.finops, d.platform, logger)
	require.NoError(t, err)
	return h
}
...
func withPlatformLogsService(s service.PlatformLogsQuerier) func(*handlerTestDeps) {
	return func(d *handlerTestDeps) { d.platform = s }
}
